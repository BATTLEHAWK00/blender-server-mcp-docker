...
async def maybe_prompt_for_consent(ctx: Any) -> str:
    ...
    """
    # A deployment-level opt-out must never trigger a client interaction.
    # Waiting for elicitation here prevents otherwise completed tools returning
    # in clients that advertise elicitation but cannot answer the dialog.
    if any(
        os.environ.get(name, "").lower() in ("true", "1", "yes", "on")
        for name in (
            "DISABLE_TELEMETRY",
